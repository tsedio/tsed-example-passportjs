...
export class CalendarCtrl {
  ...
  @Authorize("basic")
  async get(@Req("user") user: User, @Required() @PathParams("id") id: string): Promise<Calendar> {
  ...
  @Authorize("basic")
  create(@Req("user") user: User, @BodyParams() calendar: CalendarCreation): Promise<Calendar> {
  ...
  @Authorize("basic")
  async update(@Req("user") user: User, @PathParams("id") @Required() id: string, @BodyParams() @Required() calendar: CalendarCreation) {
  ...
  @Returns(404).Description("Calendar not found")
  @Authorize("basic")
  async remove(@Req("user") user: User, @BodyParams("id") @Required() id: string): Promise<void> {
